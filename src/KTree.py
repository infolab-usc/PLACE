...
from Utils import rect_area, is_rect_cover
...
class KTree(object):
    ...
    def leafCover(self, loc):
        ...
        while len(queue) > 0:
            ...
            if curr.n_isLeaf is True:
                ...
                queue.extend([curr.nw, curr.ne, curr.sw, curr.se])

    """
    return a dictionary with key = loc_id, value = {user_id, frequency}
    """
    def loc_users(self):
        locs = {}
        queue = deque()
    ...
    def debug(self, node):
        if node is None:
            return 0
        if node.n_isLeaf and node.n_data != None and node.n_data.shape[1] != 0:
            print node.n_data.shape[1], len(node.users), node.users
